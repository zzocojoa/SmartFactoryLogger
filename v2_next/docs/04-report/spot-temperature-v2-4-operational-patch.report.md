...
- Actual server CSV validator replay PASS after tightening the validator contract for stale/not-used rows: `temperature_value_origin=none` may retain a finite `spot_temperature_observed_c` only when `spot_source_freshness=stale`, `temperature_output_status` is blank or `stale`, and either `spot_cache_status=available_not_used` or `spot_cache_status=expired` with `spot_raw_validity=valid_temperature`. This preserves diagnostic observed value while keeping legacy `Temperature` blank.
...
- PR #81 server-PC Count 3 smoke evidence level: operator-provided server-PC live PowerShell output, not independent verifier direct access to the onsite server `%APPDATA%\SmartFactoryLogger\logs\test_data` path. The operator ran the smoke after installing the PR #81 merged-master NSIS build on the server PC.
- PR #81 server-PC Count 3 smoke PASS: latest non-empty live CSV selected by the operator was `Factory_Integrated_Log_v2_20260627_185112.csv`, `Length=3,269,336`, with `process-phase-candidate-v2` recorded on 2,848 imported rows. Count 3 totals were `count3_total=1,086`, `count3_production_stable=0`, and `count3_production_stabilizing=762`.
- PR #81 server-PC Count 3 tail sample PASS: representative rows `sample_seq=1058..1087` had `Count=3`, `Speed=4.9..5.0`, `MainPress=139.9..141.6 bar`, `extruder_process_state_online=extruding`, `process_phase_rule_version=process-phase-candidate-v2`, `process_phase_candidate=production_stabilizing`, populated `process_segment_id=seg_2e7bb6c6f773d257`, and blank `changeover_candidate_id`.
- PR #81 server-PC smoke verdict: PASS for the Count 3 production-motion policy. The operator-provided live CSV output proves no observed Count 3 v2 row was classified as `production_stable`; the production-motion Count 3 tail sample was classified as `production_stabilizing`.
- CSV write/watch smoke evidence level: operator-provided server-PC live PowerShell output, not independent verifier direct access to the onsite server `%APPDATA%\SmartFactoryLogger\logs\test_data` path. The operator ran a 5-minute watch after restarting the packaged server application.
- CSV write/watch smoke PASS: active CSV `Factory_Integrated_Log_v2_20260627_190605.csv` grew during the watch while `/health` stayed healthy. `/health` `rows_total` increased from `114` to `1,567`, `last_sample_seq` increased from `114` to `1,567`, CSV length increased from `44,208` to `1,908,207` bytes, imported CSV `row_count` increased from `111` to `1,567`, and `last_row_sample_seq` increased from `111` to `1,567`.
- CSV write/watch metadata/import PASS: every sampled row reported `metadata_exists=True` and `import_csv_ok=True`; latest observed row values progressed to `last_row_count_value=10` and `last_row_phase=unknown` by the end of the watch.

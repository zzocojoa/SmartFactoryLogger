...
Canary identity v5 이하 키트는 패킷 파일의 시각 역행과 일시적인 감시 조회 실패를
확정 제품 실패와 충분히 분리하지 못하므로 다시 실행하지 않는다. identity v6는
관찰 종료 시 원자적 완료 요청을 전달하고, 패킷을 안정적인 시각순으로 분석하면서
원래 캡처 순서의 역행 크기와 initial SYN 영향 여부를 별도 증거로 남긴다. 전용 오류
감시는 조회 예외의 원인을 private 증거에 기록하고 복구 여부와 최대 감시 공백을
최종 판정에 연결한다.
...
packet 결과에는 인터페이스별 집계, 중복 SYN, 시각 역행 횟수·최대 크기, initial SYN
역행 영향, 양방향 RST, 관찰 범위 밖 제외 수, 원본·중복 제거·monotonic 보정 재사용
간격이 포함된다. 시각순 정렬이나 clock calibration 상태가 확정되지 않으면 74초대를
제품 결함으로 단정하거나 기준을 완화하지 않고 `EVIDENCE_HOLD`한다. 역행이 관찰돼도
`timestamp-sorted-stable-v1` 정렬과 monotonic 보정이 완료된 경우에는 보정 결과로
판정하고 원래 역행 집계는 제한사항 증거로 계속 보존한다.

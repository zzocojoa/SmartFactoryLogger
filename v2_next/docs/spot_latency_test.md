...
```powershell
$times=1..20|%{ (Measure-Command { Invoke-WebRequest -UseBasicParsing -Uri ("http://127.0.0.1:8000/api/spot/image.jpg?ts="+[DateTime]::UtcNow.Ticks) -TimeoutSec 5 | Out-Null }).TotalMilliseconds; Start-Sleep -Milliseconds 200 }; $s=$times|Measure-Object -Average -Minimum -Maximum; "avg=$([Math]::Round($s.Average,1))ms min=$([Math]::Round($s.Minimum,1))ms max=$([Math]::Round($s.Maximum,1))ms"
```
...
PS C:\WINDOWS\system32> $times=1..20|%{ (Measure-Command { Invoke-WebRequest -UseBasicParsing -Uri ("http://127.0.0.1:8000/api/spot/image.jpg?ts="+[DateTime]::UtcNow.Ticks) -TimeoutSec 5 | Out-Null }).TotalMilliseconds; Start-Sleep -Milliseconds 200 }; $s=$times|Measure-Object -Average -Minimum -Maximum; "avg=$([Math]::Round($s.Average,1))ms min=$([Math]::Round($s.Minimum,1))ms max=$([Math]::Round($s.Maximum,1))ms"

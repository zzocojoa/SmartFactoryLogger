...
1. **Git 구조 개편** (참고: `마이그레이션_전략.md`)
   - `v1_legacy` 폴더 생성 및 기존 코드 이동.
   - `v2_next` 폴더 생성.
2. **프로젝트 초기화 (Scaffolding)**
   - **백엔드**: `v2_next` 안에 Python 가상환경 생성 및 필요
     라이브러리(FastAPI, Uvicorn) 설치.
   - **프론트엔드**: `v2_next` 안에 React 프로젝트 생성
...
1. **Step 1: 통합 빌드 (Bundling)**
   - 백엔드(Python) -> 실행 파일(`server.exe`)로 변환.
   - 프론트엔드(React) -> 정적 파일(HTML/JS)로 변환.

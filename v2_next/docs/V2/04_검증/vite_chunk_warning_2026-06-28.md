...
Main behavior risk is timezone data range. If the dashboard later needs far historical or far future timezone conversions, remove the `moment-timezone` alias and accept the larger chunk or choose a wider bundled dataset such as `moment-timezone-with-data-1970-2030.js`.

Rollback path:

- Revert the `frontend/vite.config.ts` commit.

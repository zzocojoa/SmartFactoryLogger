...
param(
    ...
    [ValidateSet(900.0,7200.0)]
    [double]$MinimumObservationSeconds = 7200.0,
...
function Get-StrictCounterValue {
    ...
    if (($value -isnot [int] -and $value -isnot [long]) -or $value -lt 0) {
        throw "$Context counter must be a nonnegative JSON integer: $Name"
    }
    return [long]$value
}
...
function Assert-RollbackBaselineEvidence {
    ...
    if ($rollbackVersion -ceq [string]$Identity.product.version) {
        throw "rollback version must differ from the candidate version"
    }
...
function Test-OperatorVisualConfirmationEligible {
    param(
        ...
        [double]$MinimumElapsedSeconds = 7200.0
    )
    ...
    return [double]$value -ge $MinimumElapsedSeconds
}
    ...
    $snapshot["source_port_recent_request_failure_events"] = @(
        foreach ($event in @(
            ...
            [pscustomobject][ordered]@{
                ...
                correlation_id = Get-OptionalProperty $event "correlation_id"
...
function Get-CumulativeFailureCounterSnapshot {
    ...
    foreach ($name in @(Get-CumulativeFailureCounterNames)) {
        $snapshot[$name] = [int64](Get-StrictCounterValue `
            -InputObject $Image `
            -Name $name `
...
function Assert-FailureCounterDeltas {
    ...
    $deltas = [ordered]@{}
    foreach ($name in @(Get-CumulativeFailureCounterNames)) {
        $beforeValue = [int64](Get-StrictCounterValue `
            -InputObject $BeforeImage `
            -Name $name `
            -Context "$Stage baseline counters")
        $afterValue = [int64](Get-StrictCounterValue `
...
function Get-InstalledState {
    ...
    # Liveness uses local config counter deltas; never add an image request.
    ...
    return [pscustomobject][ordered]@{
        ...
        live_image_probe_performed = $false
        image = $image
    }
}
...
function Get-ImageLivenessSnapshot {
    ...
    $health = Invoke-RestMethod "http://127.0.0.1:8000/health" -TimeoutSec 5
    if ([string]$health.app_version -cne $ExpectedProductVersion) {
...
function Assert-EvidenceFiles {
    ...
    if ($prerequisiteResult -eq "PENDING_SERVER_VALIDATION") {
        if (
            $Identity.prerequisite_15m.full_120m_allowed -isnot [bool] -or
            $Identity.prerequisite_15m.full_120m_allowed -ne $false -or
            $Identity.production_promotion_allowed -isnot [bool] -or
            $Identity.production_promotion_allowed -ne $false
        ) {
            throw "pending 15-minute validation must keep 120m and production disabled"
        }
    ...
    throw "This pending v1026 kit cannot accept historical PASS evidence. A new binding review is required."
}
...
function Invoke-EvidenceEvaluation {
    ...
    return [pscustomobject][ordered]@{
        result = $resultName
        evaluation_scope = "UNBOUND_OFFLINE_SUMMARY_EVALUATION"
        product_identity_binding_verified = $false
        raw_source_integrity_verified = $false
        observation_approval_granted = $false
...
function Invoke-SelfTest {
    ...
    if (
        $snapshot.source_port_recent_request_failure_events[0].correlation_id -cne
            "transport:0123456789abcdef0123456789abcdef"
    ) {
        throw "self-test safe failure event omitted transport correlation_id"
    }
    ...
    $before = [pscustomobject]@{
        ...
        backend_pid = 10
        port_8000_owner = 10
        build_git_commit = "33c8db839e4e8aa7813da4da37bfe914f3a82294"
    ...
    $identity = [pscustomobject]@{
        product = [pscustomobject]@{ version = "1.0.23" }
    ...
    $pendingIdentity = [pscustomobject]@{
        ...
        prerequisite_15m = [pscustomobject]@{
            result = "PENDING_SERVER_VALIDATION"; full_120m_allowed = $false
            evidence_files = @()
        }
    }
    Assert-EvidenceFiles -Identity $pendingIdentity -EvidenceRoot "C:\unused"
    $pendingIdentity.prerequisite_15m.result = "PASS"
    $pastRejected = $false
    try { Assert-EvidenceFiles -Identity $pendingIdentity -EvidenceRoot "C:\unused" }
    catch { $pastRejected = $_.Exception.Message -like "This pending v1026 kit*" }
    if (-not $pastRejected) { throw "self-test accepted historical PASS" }
...
# This source port is an offline review candidate, not a server launcher.
# Keep this before any live query, directory creation, collector or preflight.
[pscustomobject]@{
    result = "V1026_SERVER_LAUNCH_BINDING_REQUIRED"

# Revision r2: fresh backup/source verification and post-consent launch boundary.
# Historical r1 bytes and receipts are retained; this is not server execution approval.
[CmdletBinding()]
...
$dataRoot = 'C:\Users\user\AppData\Roaming\SmartFactoryLogger'
$profileRoot = 'C:\Users\user\AppData\Roaming\smart-factory-logger-v2'
...
function AssertStopped {
    # Enumerate first: no matching process/listener is normal; query failure is not.
    $all = @(Get-Process -ErrorAction Stop)
    Need (@($all | Where-Object { $_.ProcessName -ieq 'smart-factory' }).Count -eq 0) 'app-not-stopped'
    Need (@($all | Where-Object { $_.ProcessName -ieq 'SmartFactoryBackend' }).Count -eq 0) 'backend-not-stopped'
    $connections = @(Get-NetTCPConnection -ErrorAction Stop)
    Need (@($connections | Where-Object { $_.LocalPort -eq 8000 -and $_.State -eq 'Listen' }).Count -eq 0) 'port-not-stopped'
}
...
# r2 adds fresh, read-only cold-boundary checks. The shipped r1 stays unchanged.
function InitializeColdVerifier {
    Need ($null -eq ('SflColdBackupV1.Engine' -as [type])) 'cold-engine-session-not-fresh'
    $path = Join-Path $PSScriptRoot 'cold-backup-core.dll'
    $null = FileFact $path '3B8B3649C623EB8A7112C762D95A00AD9745231291C50CCC2B66E0AA2C7F8A71' 19456
    [void][Reflection.Assembly]::Load([IO.File]::ReadAllBytes($path))
    Need ($null -ne ('SflColdBackupV1.Engine' -as [type])) 'cold-engine-type'
}

function ReadOriginalStateFact {
    param([string]$Path)
    ...
    try {
        ...
        $before = [pscustomobject]@{
            bytes=$stream.Length;creation_utc_ticks=$info.CreationTimeUtc.Ticks
            last_write_utc_ticks=$info.LastWriteTimeUtc.Ticks;attributes=[int]$info.Attributes
            source_sddl=[IO.File]::GetAccessControl($Path,$sections).GetSecurityDescriptorSddlForm($sections)
        }
        Need ($stream.Length -le 16777216) 'cold-state-size'
        $hash = HashStream $stream
        $info.Refresh()
        Need ($stream.Length -eq $before.bytes -and
            $info.CreationTimeUtc.Ticks -eq $before.creation_utc_ticks -and
            $info.LastWriteTimeUtc.Ticks -eq $before.last_write_utc_ticks -and
            [int]$info.Attributes -eq $before.attributes -and
            [IO.File]::GetAccessControl($Path,$sections).GetSecurityDescriptorSddlForm($sections) -ceq $before.source_sddl) 'cold-state-raced'
        $before | Add-Member -NotePropertyName sha256 -NotePropertyValue $hash
        return $before
    } finally {
        # Release before launching the installer/app; do not prevent normal state writes.
        $stream.Dispose()
    }
}

function AssertOriginalStateMap {
    $map = ReadJson ($backupWorkRoot+'\state-map.json')
    Need ((Property $map 'schema_version') -ceq 'v1025-minimal-state-map-v1') 'cold-state-schema'
    Need (-not (Bool $map 'original_files_overwritten') -and -not (Bool $map 'original_files_deleted')) 'cold-state-mutated'
    $rows = @(Property $map 'files')
    Need ($rows.Count -eq 8) 'cold-state-count'
    $seen = @{}
    foreach ($row in $rows) {
        $name = [string](Property $row 'name')
        Need ($name -cin @($stateSpecs.Keys) -and $stateSpecs[$name] -and -not $seen.ContainsKey($name)) 'cold-state-member'
        $seen[$name] = $true
        $source = $dataRoot+'\'+$name
        $staged = $backupWorkRoot+'\state-source\'+$name
        Need ((Property $row 'source') -ceq $source -and (Property $row 'staged') -ceq $staged) 'cold-state-path'
        $fact = ReadOriginalStateFact $source
        foreach ($field in @('bytes','creation_utc_ticks','last_write_utc_ticks','attributes','source_sddl','sha256')) {
            Need ((Property $fact $field) -ceq (Property $row $field)) ('cold-state-drift-'+$field.Replace('_','-'))
        }
        [SflColdBackupV1.Engine]::OrdinaryData($staged)
        Need ([IO.FileInfo]::new($staged).Length -eq $fact.bytes -and
            [SflColdBackupV1.Engine]::FileHash($staged,16777216) -ceq $fact.sha256) 'cold-state-staged-drift'
    }
    foreach ($entry in $stateSpecs.GetEnumerator()) {
        if ($entry.Value) { Need ($seen.ContainsKey($entry.Key)) 'cold-state-missing' }
        else {
            $absent = $dataRoot+'\'+$entry.Key
            [SflColdBackupV1.Engine]::Plain($absent,$true)
            Need (-not [IO.File]::Exists($absent) -and -not [IO.Directory]::Exists($absent)) 'cold-state-absent-appeared'
        }
    }
}

function AssertColdBackupContents {
    param([object]$Result)
    AssertStopped
    AssertOriginalStateMap
    # r0/r1/r2 are the exact source order used by the pinned backup producer.
    $sources = [string[]]@($profileRoot,$layoutsRoot,($backupWorkRoot+'\state-source'))
    $guard = [Action]{ AssertStopped }
    $progress = [Action[string,long,long,long]]{ param($name,$bytes,$files,$elapsed) }
    $engine = [SflColdBackupV1.Engine]::new($sources,$guard,$progress)
    foreach ($leaf in @('backup','restore')) {
        $totals = $engine.Verify(($backupWorkRoot+'\files.manifest.tsv'),$backupManifestHash,($backupWorkRoot+'\'+$leaf),$null,$false)
        Need ($totals.Files -eq [long](Property $Result 'source_file_count') -and
            $totals.Directories -eq [long](Property $Result 'source_directory_count') -and
            $totals.Bytes -eq [long](Property $Result 'source_bytes')) 'cold-backup-counts'
    }
    AssertOriginalStateMap
    AssertStopped
}

function InvokeColdCheckedLaunch {
    param([scriptblock]$ColdCheck,[scriptblock]$StoppedCheck,[scriptblock]$Launch)
    & $ColdCheck | Out-Null
    & $StoppedCheck | Out-Null
    # No prompt, copy, compile, receipt, or other work between this gate and launch.
    & $Launch
}
...
function AssertBackup {
    ...
    AssertColdBackupContents $result
    return $result
}
...
function LaunchInstallerStandard {
    ...
    try{InvokeColdCheckedLaunch { $null=AssertBackup } { AssertStopped } { $shell.ShellExecute($copy,'',$script:launchRoot,'open',1) }}finally{if($null-ne$shell){[void][Runtime.InteropServices.Marshal]::FinalReleaseComObject($shell)}}

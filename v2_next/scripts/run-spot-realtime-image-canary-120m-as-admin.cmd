...
echo.
echo [CHECK] Running fail-closed identity, rollback, 15-minute evidence, and pktmon preflight.
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%~dp0invoke-spot-realtime-image-canary-120m.ps1" -ReleaseKitRoot "%~dp0.." -PreflightOnly
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%~dp0invoke-spot-realtime-image-canary-120m.ps1" -ReleaseKitRoot "%~dp0.."
...
if "%SFL_CANARY_EXIT%"=="0" (
  ...
  echo [HOLD] Runtime rollback was not automatically performed, but packet evidence is incomplete.
  ...
  echo [ROLLBACK REQUIRED] Preserve evidence, close SmartFactoryLogger normally, then use the verified v1.0.16 installer.
) else (

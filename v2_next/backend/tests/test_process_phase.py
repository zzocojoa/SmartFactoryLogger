...
class ProcessPhaseCandidateTests(unittest.TestCase):
    def test_extruding_maps_to_production_stable_without_spot_input(self) -> None:
        decision = derive_process_phase_candidate(
            fresh_source(speed=5.0, press=35.0, count=12, extruder_process_state_online="extruding")
        )
    ...
    def test_low_count_low_motion_maps_to_setup_candidate(self) -> None:
        decision = derive_process_phase_candidate(
            fresh_source(speed=0.0, press=0.0, count=1, extruder_process_state_online="unknown")
        )
    ...
    def test_low_count_high_motion_maps_to_setup_alignment_candidate(self) -> None:
        cases = (
            fresh_source(speed=1.0, press=35.0, count=0, extruder_process_state_online="unknown"),
            fresh_source(speed=0.0, press=35.0, count=2, extruder_process_state_online="extruding"),
        )
        for case in cases:
    ...
    def test_count_three_high_motion_maps_to_production_stabilizing(self) -> None:
        decision = derive_process_phase_candidate(
            fresh_source(speed=1.0, press=35.0, count=3, extruder_process_state_online="extruding")
        )
    ...
    def test_count_four_high_motion_can_promote_to_production_stable(self) -> None:
        decision = derive_process_phase_candidate(
            fresh_source(speed=1.0, press=35.0, count=4, extruder_process_state_online="extruding")
        )

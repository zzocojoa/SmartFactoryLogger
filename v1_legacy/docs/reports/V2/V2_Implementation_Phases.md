...
## Phase 0. 요구사항 및 현장 진단
- 현장 운영 시나리오, 장애 유형, 데이터 품질 요구를 수집.
- 비기능 요구사항 정의: 응답 시간, 가동률, 보안, 업데이트.
- V1 재사용 범위와 V2 범위(기능/화면/API)를 확정.
...
## Phase 4. 운영 및 검증
- 파일럿 운영(2~4주)으로 안정성/알림/로그를 검증.
- 성능/안정성 튜닝과 장애 대응 매뉴얼을 정리.
- 운영자 교육 및 배포 체크리스트를 현장에 적용.
- 장애 재현 테스트: 네트워크 끊김, NAS 미연결, PLC 응답 지연을 시뮬레이션하고 복구 시간을 측정.
- 관측성 체계 구축: 헬스 체크, 오류 알림, 로그/메트릭 수집과 현장 연락 프로토콜을 정의.
...
- [ ] **코드 서명 인증서** 준비 (SmartScreen 대응)
- [ ] **설치/업데이트 경로**: ProgramData 기반 설정/로그 분리
- [ ] **NAS 접근 권한/매핑** 사전 테스트

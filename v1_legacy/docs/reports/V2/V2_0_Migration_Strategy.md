...
### 3.1. 2025 실무 보완 포인트
- 브랜치 정책: main은 V1 안정화, v2는 신규 개발, hotfix는 체리픽 기준을 명시.
- 공통 코드 분리: 수집/모델/유틸은 shared 패키지로 분리해 복사-붙여넣기 리스크를 제거.
- CI 파이프라인 분리: V1 안정성 테스트, V2 빌드/패키징 자동화.
- 데이터/설정 이관 계획: config.ini 위치 고정, 마이그레이션 스크립트, 롤백 지원.
- 릴리스/버전 규칙: Semantic Versioning, 변경 로그, 태그 전략.
- 병행 운영(dual-run) 기간과 전환 종료 조건(오류율, 데이터 누락률)을 지표로 명확히 정의한다.
- 롤백 시나리오를 문서화하고 데이터 동기 범위, 다운타임 허용치, 책임자를 지정한다.
